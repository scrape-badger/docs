---
title: "Leboncoin Scraper by ScrapeBadger — Overview"
...
```bash cURL
curl "https://scrapebadger.com/v1/leboncoin/search?text=v%C3%A9lo+%C3%A9lectrique&region_id=22&page=1" \
...
```bash
curl "https://scrapebadger.com/v1/leboncoin/search?text=iphone" \
  -H "X-API-Key: YOUR_API_KEY"
```
...
*ScrapeBadger is an independent tool and is not affiliated with, endorsed by, or sponsored by leboncoin. "leboncoin" is a trademark of LBC France SAS (part of the Adevinta group), used here only to describe the platform this scraper is designed to work with.*
