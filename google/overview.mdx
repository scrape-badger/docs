---
title: "Google Scraper by ScrapeBadger — Overview"
...
1. First request attempts a **fast HTTP fetch** with cached cookies (~0.2s fast path)
2. If a SearchGuard challenge is detected, a **browser warmup** solves it
3. Subsequent requests reuse the warm cookies over HTTP for speed
...
<Tabs>
  ...
  <Tab title="Shopping">
    ...
    | `GET /v1/google/shopping/product/click` | Resolve merchant URL per product (lightweight HTTP lookup) |
  </Tab>
...
Per-endpoint credit pricing is configured by ScrapeBadger admins and returned live from `GET https://scrapebadger.com/api/public/pricing` (uncached, updates immediately). The public pricing page and each product landing page at [scrapebadger.com](https://scrapebadger.com) show the authoritative current rate. See [Credits & Pricing](/credits-and-pricing) for plan-level rates.

<Tip>
  Query `GET /api/public/pricing` in your integration to cache the current per-endpoint cost — don't hardcode credit numbers, they can be adjusted.
</Tip>
...
*ScrapeBadger is an independent tool and is not affiliated with, endorsed by, or sponsored by Google. "Google," "Google Maps," "Google Trends," and other Google product names are trademarks of Google LLC, used here only to describe the platforms this scraper is designed to work with.*
