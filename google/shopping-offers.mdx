...
async with ScrapeBadger(api_key="YOUR_API_KEY") as client:
    result = await client.google.shopping_offers(barcode="0711719541028")
...
const result = await client.google.shoppingOffers({ barcode: "0711719541028" });
...
async with ScrapeBadger(api_key="YOUR_API_KEY") as client:
    result = await client.google.shopping_offers(barcode="9300633572457", gl="au")
```

```javascript Node.js
const result = await client.google.shoppingOffers({
