...
<Note>
Windsurf has a maximum of 100 tools across all MCP servers. ScrapeBadger exposes 216, so Windsurf will only load a subset — disable the tools you don't need in Windsurf's MCP panel, or use a client without a tool cap for full coverage.
</Note>
