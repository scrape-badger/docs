---
title: "Booking.com Scraper by ScrapeBadger — Overview"
description: "Scrape Booking.com hotel search with filters and sorting, destination autocomplete, full property detail, complete room rate tables, and paginated guest reviews — worldwide, with structured JSON responses."
---

## Scraper API for Booking.com
...
rooms, house rules, score breakdown, photos and guest Q&A — pull its complete
rate table for any dates, and page through guest reviews with language and
guest-type filters. ScrapeBadger handles sessions and parsing automatically —
no Booking.com account or API key required.
...
<CardGroup cols={3}>
  ...
  <Card title="Full Rate Table" icon="bed">
    Every rate on every room type for your dates — 30+ rates across 5 room types in one call, with per-room facilities, alternative bed layouts and meal plans.
  </Card>
...
| Room sizes | Follows `language` | [`/rooms`](/api-reference/endpoint/booking/get-rooms) reports `area_unit` as `SQUARE_METER` or `SQUARE_FEET` depending on the requested locale. |
...
<Tip>
  ...
  [`/rooms`](/api-reference/endpoint/booking/get-rooms) and
  [`/reviews`](/api-reference/endpoint/booking/get-reviews).
</Tip>
...
| [`/v1/booking/properties/{country_code}/{slug}/rooms`](/api-reference/endpoint/booking/get-rooms) | GET | Every room type with its full rate table for given dates |
...
*ScrapeBadger is an independent tool and is not affiliated with, endorsed by, or sponsored by Booking.com. "Booking.com" is a trademark of Booking.com B.V., used here only to describe the platform this scraper is designed to work with.*
