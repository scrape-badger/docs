...
# Command Reference ## Account Commands ### `scrapebadger auth` Set up or view your API key.
...
### `scrapebadger credits` Check your credit balance. ### `scrapebadger logout` Remove saved API key from config. ---

## Twitter Commands All Twitter commands are under `scrapebadger twitter`. ### Users
...
## Google Commands All Google commands are under `scrapebadger google`. **29 subcommands** cover all 16 Google product APIs. ### Search
...
</Tip> ### Patents, Scholar, Finance, Products
...
### Common Options Every google command supports `--output/-o` (`json`, `csv`, `table`, `markdown`) and `--fields/-f` (comma-separated field list).
...
## Vinted Commands All Vinted commands are under `scrapebadger vinted`.
...
| `--catalogs` | | Comma-separated catalog (category) IDs — the `catalog[]` value in a Vinted category URL |
| `--colors` | | Comma-separated color IDs |
| `--statuses` | | Comma-separated condition IDs |
| `--order` | | Sort order: relevance, newest_first, price_low_to_high, price_high_to_low | ---

## Web Scraping Commands All web commands are under `scrapebadger web`.
...
Supply exactly one of `--image`, `--image-url` or `--image-base64`. JPEG, PNG and
WebP up to 5 MiB are supported. A successful search costs 10 credits.
See [Vinted image search](/vinted/image-search) for all filters and pagination.
