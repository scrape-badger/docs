---
title: "eBay Scraper by ScrapeBadger — Overview"
description: "Scrape eBay active listings, item details, reviews, and seller data across 18 marketplaces with structured JSON responses."
---

## Scraper API for eBay

Search active listings by keyword or by image, fetch full item detail, list reviews, and retrieve seller profiles, storefronts, and feedback from eBay — one of the world's largest online marketplaces. The API handles authentication, anti-bot bypass, and marketplace routing automatically.
...
<CardGroup cols={3}>
  ...
  <Card title="18 Marketplaces, One API" icon="globe">
    Search, item detail, reviews and seller data resolve across every supported
    marketplace from a single `domain` parameter.
  </Card>
...
<Note>
  eBay no longer exposes public **watcher counts**, so the `watchers` field is often `null` on search, seller-items, and item-detail responses. `sold_count` remains available on item detail and is the reliable signal for demand. Completed/sold **price history** is available via [`/v1/ebay/completed`](/api-reference/endpoint/ebay/completed), which returns items that have sold with their sale price and date.
</Note>
...
| [`/v1/ebay/search/by-image`](/api-reference/endpoint/ebay/search-by-image) | POST | Search active listings by image — eBay's own visual search |
| [`/v1/ebay/completed`](/api-reference/endpoint/ebay/completed) | GET | Search completed (sold) listings — sold-price history |
...
<CardGroup cols={2}>
  ...
  <Card title="Item Detail" icon="tag" href="/api-reference/endpoint/ebay/get-item-detail">
