...
Search active listings, fetch full item detail, list reviews, and retrieve seller profiles, storefronts, and feedback from eBay — one of the world's largest online marketplaces. The API handles authentication, anti-bot bypass, and marketplace routing automatically.

<Warning>
  **Completed/sold listings were withdrawn by eBay in August 2026.** Viewing sold
  prices now requires a signed-in eBay account, so
  [`/v1/ebay/completed`](/api-reference/endpoint/ebay/completed) returns HTTP 410 and is
  not billed. Every other eBay endpoint is unaffected.
</Warning>
...
<Note>
  eBay no longer exposes public **watcher counts**, so the `watchers` field is often `null` on search, seller-items, and item-detail responses. `sold_count` remains available on item detail and is the reliable signal for demand. Completed/sold **price history** is no longer obtainable anonymously — see the note below.
</Note>
...
| [`/v1/ebay/completed`](/api-reference/endpoint/ebay/completed) | GET | **Deprecated** — returns 410, not billed |
...
|----------|------|
| Search | 5 credits |
| Completed / sold | *deprecated — not billed* |
