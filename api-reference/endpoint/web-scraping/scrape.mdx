...
<ParamField body="engine" type="string" default="auto">
  ...
  |-------|-------------|------|
  | `auto` | Selects `curl_cffi` by default, escalates to `browser` if JS rendering is needed | Varies |
  | `curl_cffi` | HTTP client with Chrome TLS fingerprint impersonation | 1 credit |
  | `rnet` | Backward-compatible alias for `curl_cffi` | 1 credit |
  | `browser` | Headless browser (Patchright/Camoufox) with full JS rendering | 5 credits |
  | `patchright` | Backward-compatible alias for `browser` | 5 credits |
  | `windows_chrome` | Real Chrome on a remote Windows VPS via CDP | 10 credits |
</ParamField>
