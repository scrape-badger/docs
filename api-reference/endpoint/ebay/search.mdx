...
<ParamField query="location" type="string">
  ...
  marketplace. Two things follow, and both matter for price research: a national
  price series picks up sales that never happened in that country, and a foreign
  listing's price is shown **converted** into the marketplace's currency rather
  than the amount it actually sold for. `location=domestic` removes both.
</ParamField>
...
<ResponseField name="results" type="array">
  ...
  <Expandable title="SearchResult object">
    ...
    <ResponseField name="sold_date" type="string">Sale date text as rendered by eBay, e.g. `2 Jul 2026` (completed/sold cards only; localized on non-English marketplaces, nullable).</ResponseField>
