---
title: "Completed / Sold Listings (deprecated)"
openapi: "GET /v1/ebay/completed"
description: "Deprecated — eBay requires a signed-in account for sold listings. This endpoint returns 410."
---

<Warning>
  **This endpoint is deprecated and no longer available.**

  eBay requires a signed-in account to view completed/sold listings. The desktop
  results page has required one since 25 July 2026, and the mobile page that still
  served them closed on 19 August 2026 across every marketplace. There is no
  anonymous path left, so this API does not provide one.

  Every request now returns **HTTP 410** with an `endpoint_deprecated` body, and
  **is not billed**. Active-listing endpoints — [`/v1/ebay/search`](/api-reference/endpoint/ebay/search),
  item detail, sellers, categories and autocomplete — are unaffected and fully supported.
</Warning>

## What to use instead

For current prices, use [`/v1/ebay/search`](/api-reference/endpoint/ebay/search), which
returns active listings with the same result shape. It does not carry closing prices —
eBay no longer exposes those anonymously — so sold-price history is not available through
this API by any route.

## Response

```json
{
  "error": "endpoint_deprecated",
  "endpoint": "completed",
  "deprecated": true,
  "message": "This endpoint is deprecated and no longer available. eBay now requires a signed-in account to view completed/sold listings, which the ScrapeBadger eBay API intentionally does not provide. Active-listing endpoints (search, items, sellers, categories, autocomplete) remain fully supported — use /v1/ebay/search for current prices."
}
```

The response also carries `Deprecation: true` and a `Sunset` header.

---

<Note>
  The reference below describes the endpoint as it behaved before 19 August 2026 and is
  retained so existing integrations can identify what they were calling. None of it is
  served any more.
</Note>

The completed-listings endpoint returned **sold** items — the closing prices buyers actually paid. Every result had `sold` set to `true` on the response envelope.

## Query Parameters

<ParamField query="query" type="string" required>
  Search keywords. Matches against listing titles.
</ParamField>

<ParamField query="domain" type="string" default="com">
  eBay marketplace domain TLD or alias. See [`/v1/ebay/markets`](/api-reference/endpoint/ebay/list-markets) for all supported values.

  Examples: `com`, `co.uk`, `de`, `fr`, `com.au`
</ParamField>

<ParamField query="category_id" type="string">
  Restrict results to an eBay category id. Use [`/v1/ebay/categories`](/api-reference/endpoint/ebay/list-categories) to look up ids.
</ParamField>

<ParamField query="page" type="integer" default={1}>
  Page number for paginated results. Range: `1` - `1000`.
</ParamField>

<ParamField query="per_page" type="integer">
  Results per page. Clamped to one of `60`, `120`, or `240`.

  Completed/sold results are served from eBay's mobile listing grid, which
  returns fewer cards per page than the requested value — roughly `25` when
  `per_page` is omitted, and about 5/6 of the requested value otherwise. Treat
  it as an upper bound and read the actual count from `results`.
</ParamField>

<ParamField query="sort_by" type="string" default="best_match">
  Sort order for results.

  | Value | Description |
  |-------|-------------|
  | `best_match` | Best match (default) |
  | `ending_soonest` | Ended soonest first |
  | `newly_listed` | Most recently ended |
  | `price_low_to_high` | Lowest sold price first |
  | `price_high_to_low` | Highest sold price first |
</ParamField>

<ParamField query="condition" type="string">
  Item condition filter (`new`, `open_box`, `refurbished`, `used`, `for_parts`).
</ParamField>

<ParamField query="min_price" type="number">
  Minimum sold-price filter in the marketplace's local currency.
</ParamField>

<ParamField query="max_price" type="number">
  Maximum sold-price filter in the marketplace's local currency.
</ParamField>

## Response

Identical shape to [`/v1/ebay/search`](/api-reference/endpoint/ebay/search), except `sold` is `true` and each result's `price` reflects the **final sold price**.

<ResponseField name="query" type="string">The search query that was executed.</ResponseField>
<ResponseField name="domain" type="string">Marketplace domain that was searched.</ResponseField>
<ResponseField name="sold" type="boolean">Always `true` for completed/sold listings.</ResponseField>
<ResponseField name="results" type="array">
  Array of sold listings (same `SearchResult` shape as active search).

  <Expandable title="SearchResult object">
    <ResponseField name="position" type="integer">Position on the page (1-based).</ResponseField>
    <ResponseField name="item_id" type="string">eBay listing item id.</ResponseField>
    <ResponseField name="title" type="string">Listing title.</ResponseField>
    <ResponseField name="url" type="string">Full URL to the listing.</ResponseField>
    <ResponseField name="image" type="string">Primary image URL.</ResponseField>
    <ResponseField name="price" type="object">Final **sold** price with `value`, `currency`, `symbol`, `raw`.</ResponseField>
    <ResponseField name="condition" type="string">Item condition label.</ResponseField>
    <ResponseField name="buying_format" type="string">`Buy It Now`, `Auction`, or `Best Offer`.</ResponseField>
    <ResponseField name="is_auction" type="boolean">Whether the listing was an auction.</ResponseField>
    <ResponseField name="bids" type="integer">Number of bids the auction received (nullable).</ResponseField>
    <ResponseField name="time_left" type="string">Relative time remaining, e.g. `12h 16m` (auctions only, nullable).</ResponseField>
    <ResponseField name="current_bid" type="object">Current high bid for auction listings with `value`, `currency`, `symbol`, `raw`; mirrors `price`. Null for non-auction (fixed-price) listings.</ResponseField>
    <ResponseField name="shipping" type="string">Shipping text.</ResponseField>
    <ResponseField name="location" type="string">Item location text.</ResponseField>
    <ResponseField name="sold_date" type="string">Sale date text as rendered by eBay, e.g. `2 Jul 2026`. Localized on non-English marketplaces (e.g. `Verkauft 5. Okt. 2024`), nullable.</ResponseField>
    <ResponseField name="sold_date_at" type="string">Best-effort ISO date parsed from `sold_date`, e.g. `2026-07-02`. Null when the marketplace's date format is not English.</ResponseField>
    <ResponseField name="seller_name" type="string">Seller username (nullable).</ResponseField>
    <ResponseField name="is_sponsored" type="boolean">Always `null`. eBay renders its "Sponsored" badge into every card as anti-scraping bait, so promoted placements cannot be distinguished from organic results.</ResponseField>
  </Expandable>
</ResponseField>
<ResponseField name="facets" type="object">Available filter facets keyed by name.</ResponseField>
<ResponseField name="pagination" type="object">Pagination metadata with `current_page`, `per_page`, `total_pages`, `total_results`, `has_more`.

`total_pages` and `total_results` are always `null` on this endpoint — eBay's completed-listings grid reports neither a result count nor a true page total. Use `has_more` to page instead (see below).</ResponseField>
<ResponseField name="scraped_at" type="string">ISO 8601 timestamp when the results were scraped.</ResponseField>

### Example Response

```json
{
  "query": "nintendo switch oled",
  "domain": "com",
  "sold": true,
  "results": [
    {
      "position": 1,
      "item_id": "256987654321",
      "title": "Nintendo Switch OLED Model White - Used",
      "url": "https://www.ebay.com/itm/256987654321",
      "image": "https://i.ebayimg.com/images/g/xyz/s-l500.jpg",
      "price": { "value": 215.5, "currency": "USD", "symbol": "$", "raw": "$215.50" },
      "condition": "Used",
      "buying_format": "Auction",
      "is_auction": true,
      "bids": 14,
      "time_left": null,
      "current_bid": { "value": 215.5, "currency": "USD", "symbol": "$", "raw": "$215.50" },
      "shipping": "Free shipping",
      "location": "United States",
      "sold_date": "18 Jun 2026",
      "sold_date_at": "2026-06-18",
      "seller_name": "gamerdeals99",
      "is_sponsored": null
    }
  ],
  "facets": {},
  "pagination": { "current_page": 1, "per_page": 60, "total_pages": null, "total_results": null },
  "scraped_at": "2026-06-21T12:00:00Z"
}
```

## Fetching every sold listing (historical)

`has_more` is the stop signal for bulk extraction. Increment `page` while it is `true`:

```python
page, all_sold = 1, []
while True:
    r = requests.get(
        "https://scrapebadger.com/v1/ebay/completed",
        params={"query": "iphone 13", "per_page": 240, "page": page},
        headers={"X-API-Key": API_KEY},
    ).json()

    all_sold += r["results"]
    if not r["results"] or not r["pagination"]["has_more"]:
        break
    page += 1
```

<Warning>
  Do not loop on "until the response is empty" alone. Past its last page eBay
  **clamps** — asking for page 200 of a 133-page result set re-serves page 133.
  This API detects that and returns an empty page with `has_more: false`, but a
  client that ignores `has_more` and retries forever will keep spending credits.
</Warning>

A broad sold search runs roughly 130 pages deep at `per_page=240` (~26,000 listings).
Use the largest `per_page` you can: each request costs 5 credits regardless of page
size, so `per_page=240` is about 8x cheaper per listing than the default.

<Note>
  `per_page` is an upper bound. eBay's sold grid returns about 5/6 of the requested
  value (≈200 for `240`, ≈50 for `60`) and ≈25 when omitted. Pages are contiguous —
  no listings are skipped between them.
</Note>

<Note>
  Historical pricing: each request cost **5 credits**. The endpoint now returns 410 and is not billed at all.
</Note>
