...
<ResponseField name="pagination" type="object">Pagination metadata with `current_page`, `per_page`, `total_pages`, `total_results`.

On this endpoint `total_pages` and `total_results` are always `null` — eBay's completed-listings grid reports neither a result count nor a true page total. Page through by incrementing `page` until `results` comes back empty.</ResponseField>
