...
<ResponseField name="pagination" type="object">Pagination metadata with `current_page`, `per_page`, `total_results`, `has_more`.

`total_results` is populated (the sold results page carries a match count). `total_pages` may be `null` — page with `has_more` rather than a fixed page total.</ResponseField>
