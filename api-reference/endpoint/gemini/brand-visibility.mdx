...
api: "GET https://scrapebadger.com/v1/gemini/brand-visibility"
description: "Ask Gemini a question and get a full AEO/GEO analysis of how your brand appears in the answer — mentions, position, share of voice against competitors, and whether your domain was cited."
---

Ask **the real `gemini.google.com`** a buyer-intent question and get back both
the answer **and** a structured analysis of how your brand fares in it: whether
it was mentioned, how early, how often, how it compares to named competitors,
and whether your domain was cited as a source.
...
$99–989/month subscription — here it is one 25-credit call, measured inside
Gemini specifically.
...
<ParamField query="prompt" type="string" required>
  The question to ask Gemini — typically a buyer-intent prompt like
...
<ParamField query="web_search" type="string" default="force">
  ...
  tracking usually wants Gemini to actually ground the answer. Check
...
<ResponseField name="citations" type="Citation[]">
  The sources Gemini referenced — same shape as
  [`/ask`](/api-reference/endpoint/gemini/ask).
...
<ResponseField name="web_search_triggered" type="boolean">Whether Gemini actually grounded the answer with a web search.</ResponseField>
<ResponseField name="model" type="string | null">Which model answered — a Gemini Flash-Lite build.</ResponseField>
...
```bash cURL
curl -G "https://scrapebadger.com/v1/gemini/brand-visibility" \
...
const res = await fetch(
  "https://scrapebadger.com/v1/gemini/brand-visibility",
  {
    method: "POST",
    headers: {
...
<Tip>
  Because Gemini is non-deterministic, one call is a sample, not a
