...
This endpoint reads mobile JSON for the description, photos, seller and item attributes. Fields unavailable upstream remain null. In particular, size-guide FAQ IDs are not size IDs. See [mobile data](/vinted/mobile-api) for raw plugins, related items and service prices.
