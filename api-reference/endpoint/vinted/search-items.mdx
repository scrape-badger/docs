...
`size_ids` and `material_ids` accept comma-separated filter IDs. Reuse the returned pagination `time` and a shared `search_session_id` across pages. Search item `display_title` / `display_subtitle` preserve presentation labels; `brand_title` can be null. Use item details for verified brand, category and condition IDs. See [mobile data](/vinted/mobile-api).
