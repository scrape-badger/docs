...
description: "Hashtag metadata by name, plus a paginated recent feed for the tag. Temporarily unavailable."
tag: "UNAVAILABLE"
---
...
<Warning>
  **Temporarily unavailable.** This endpoint needs the authenticated Instagram
  ...
  returns `503 {"error": "temporarily_unavailable"}` with a `Retry-After`
  header and is **not billed**. The public endpoints — [profile](/api-reference/endpoint/instagram/get-user),
  [posts](/api-reference/endpoint/instagram/user-posts), [media detail](/api-reference/endpoint/instagram/get-media)
  and [comments](/api-reference/endpoint/instagram/media-comments) — are live.
</Warning>
