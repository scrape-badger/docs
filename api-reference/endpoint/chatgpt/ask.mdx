...
<ParamField query="image_url" type="string">
  Public `http(s)` URL of an image to attach to the prompt. ChatGPT looks at the
...
<Note>
  An image ask is slower than a text one: ChatGPT uploads the picture before it
...
<ResponseField name="images" type="MediaItem[]">
  ...
  <Expandable title="MediaItem">
    ...
    <ResponseField name="title" type="string | null">Alt text, when ChatGPT supplied one.</ResponseField>
  </Expandable>
</ResponseField>
...
<Note>
  An image ChatGPT cannot read is a `400` with `"code": "invalid_image"`, raised
