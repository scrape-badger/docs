...
<ParamField query="page" type="integer" default="1">
  Offer page. Amazon lists 10 seller offers per page; page 1 also carries the Buy Box offer.
  Compare `total_offers` with the number of offers returned to know whether more pages exist.
</ParamField>
...
<ResponseField name="buybox" type="object">The Buy Box offer (page 1 only). Same shape as an offer; also the first entry of `offers`.</ResponseField>
<ResponseField name="total_offers" type="integer">Amazon's total offer count across all pages, including the Buy Box offer. Larger than the number of offers returned when more pages exist; `null` on pages after the first.</ResponseField>
