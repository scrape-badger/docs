...
| `GET /v1/youtube/videos/{video_id}/streams` | Streaming formats (muxed + adaptive); `client` selects `IOS` (default), `ANDROID`, or `WEB`. **15 credits.** |
...
<Warning>
  **Stream URLs are best-effort and exit-bound.** YouTube signs each media URL with the IP that fetched it (`ip=` in the query string) and enforces that binding on some CDN nodes, so a URL that plays from one host may return `403` from another. Fetch promptly — they carry a short `expires_in_seconds` — and treat a `403` as "re-request the streams", not as an outage. Videos YouTube serves no playable formats for (unavailable, private, or app-gated) return `404`, and are not charged.
</Warning>
