---
title: "Immobiliare.it Scraper by ScrapeBadger — Overview"
...
## Scraper API for Immobiliare.it
...
const res = await fetch(
  "https://scrapebadger.com/v1/immobiliare/search?" +
    new URLSearchParams({
      location: "Milano",
      contract: "sale",
...
```bash cURL
curl "https://scrapebadger.com/v1/immobiliare/search?location=Milano&contract=sale&category=residential&price_max=300000&rooms_min=3" \
...
```bash
curl "https://scrapebadger.com/v1/immobiliare/search?location=Milano" \
  -H "X-API-Key: YOUR_API_KEY"
```
...
*ScrapeBadger is an independent tool and is not affiliated with, endorsed by, or sponsored by Immobiliare.it. "Immobiliare.it" is a trademark of Immobiliare.it S.p.A., used here only to describe the platform this scraper is designed to work with.*
